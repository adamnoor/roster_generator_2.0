...
def write_qb_rosters(quarterback):
    ...
    count = 0
    lcl_qb_combos = []
    for element in qb_dst_combos:
        if element.qb == quarterback:
            lcl_qb_combos.append(element)
    print ("")
    print("Generating valid rosters for " + quarterback.name + ".  This may take some time...")
    all_rosters = []
    start = time.time()

    for i in lcl_qb_combos:
        for j in flex_combos:
            ...
            if salary > 50000:
                ...
                count += 1
                if count >= 1000000 and count %1000000 == 0:
                    print(str(count) + " valid rosters for " + i.qb.name + " have been generated so far")
                    end = time.time()
    ...
    print("Writing the CSV file for " + quarterback.name)
    print("")
    header = ['QB', 'RB', 'RB', 'WR', 'WR', 'WR', 'TE', 'FX', 'DST', 'Salary', 'Projection']
    data = all_rosters

    with open('generated_files/' + quarterback.name + '.csv', 'w') as f:
        writer = csv.writer(f)
